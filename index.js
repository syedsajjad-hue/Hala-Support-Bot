...
// ================= FORM URL ================= 
const PROFILE_UPDATE_FORM_URL = process.env.PROFILE_UPDATE_FORM_URL || 'https://example.com/form';
...
function issueTypeButtons() { 
    return Markup.inlineKeyboard([ 
        [Markup.button.callback('Payment Issue', 'disp_payment'), Markup.button.callback('Account Block', 'disp_account_block')], 
        [Markup.button.callback('Stuck Booking', 'disp_stuck_booking'), Markup.button.callback('Device Issue', 'disp_device_issue')], 
...
function profileUpdateButtons() { 
    return Markup.inlineKeyboard([ 
        [Markup.button.callback('Number Update', 'profile_number_update'), Markup.button.callback('Profile Picture Update', 'profile_picture_update')], 
        [Markup.button.callback('Back', 'menu_create_ticket'), Markup.button.callback('Cancel', 'nav_cancel')] 
    ]); 
} 

// ================= PROFILE NUMBER UPDATE (UNCHANGED FLOW + LINK) ================= 
...
// ================= DEVICE ISSUE (FIXED MESSAGE ONLY) ================= 
bot.action('disp_device_issue', async (ctx) => { 
    await ctx.answerCbQuery(); 
    try { await ctx.deleteMessage(); } catch (e) {} 

    return ctx.reply('Please Visit Hala Home for the Device issue. Thanks'); 
}); 

// ================= START MESSAGE (FIXED) ================= 
bot.start(async (ctx) => { 
    return ctx.reply('Welcome to the Hala Captain Support', mainMenuButtons()); 
}); 

// ================= PLACEHOLDER FOR REST OF YOUR ORIGINAL LOGIC ================= 
// (All ticketing, supabase, google sheets, alerts, dashboard logic remains unchanged in your full project)
