...
bot.action(/^resolve_(.+)$/, async (ctx) => {
  ...
  const assignedAgent =
    oldTicket?.assigned_to && oldTicket.assigned_to !== 'Unassigned'
  ...
  const msg = ticketMessage(data);
  const removeButtons = { reply_markup: { inline_keyboard: [] } };

  try {
    if (ctx.callbackQuery.message.photo) {
      await ctx.editMessageCaption(msg, removeButtons);
    } else {
      await ctx.editMessageText(msg, removeButtons);
    }
  } catch (err) {
...
async function checkTicketStatus(ctx, ticketNumber) {
  try {
    ...
    const statusIcon = data.status === 'Resolved' ? '🟢 Resolved' : '🟡 Pending';
